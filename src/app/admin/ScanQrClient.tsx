...
export default function ScanQrClient({ userName }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/80 bg-white/85 shadow-2xl shadow-emerald-100 backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Admin aktif</p>
      ...
      <div className="p-6">
        <div className="relative grid aspect-square place-items-center overflow-hidden rounded-[1.5rem] border border-slate-100 bg-slate-50">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,#bbf7d0_0,transparent_38%)] opacity-60" />
          {qr ? (
            <img src={qr} alt="QR WhatsApp pairing" className="relative z-10 h-[82%] w-[82%] rounded-2xl bg-white p-3 shadow-xl" />
            ...
            <div className="relative z-10 text-center">
              ...
              <p className="mt-4 font-bold text-slate-700">Menyiapkan QR...</p>
            </div>
          ) : state === "connected" ? (
            <div className="relative z-10 text-center">
              ...
              {connectedNumber && <p className="mt-1 text-sm text-slate-500">{connectedNumber}</p>}
            </div>
          ) : (
        ...
        <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            {sessionName && <span className="rounded-full bg-white px-3 py-1 text-slate-500 ring-1 ring-slate-200">{sessionName}</span>}
